...
The default provider is `stepfun`. For StepFun, set the API key in `STEP_API_KEY`. Never hard-code API keys in commands, scripts, skill files, logs, or generated artifacts.
...
3. Call the bundled script before giving the final answer or producing image output. Use `image` for screenshots, photos, charts, receipts, diagrams, UI states, and multi-image comparison. Use `video` for recordings, demos, timeline extraction, or video summaries. Use `generate` for text-to-image and `edit` for prompt-based image editing.
4. For perception tasks, use local-file default transport unless there is a reason to override it:
...
5. For image generation/editing, pass a clear prompt and an `--output` path. Use `--image-size`, `--steps`, `--cfg-scale`, `--seed`, `--negative-prompt`, and `--text-mode` only when the user request calls for those controls.
6. Choose `--reasoning-effort` deliberately for perception tasks:
...
1. Confirm the runtime command and environment. If local shell config may hold the provider key, source it before the script; if the default Python lacks the provider SDK, use an environment that has the dependency installed.
...
3. Missing SDK: report the dependency issue and suggest installing the provider SDK or switching to an existing Python environment with the package.
...
7. Upload failure, `500`, or connection error: preserve the exact error message. Retry only when the failure looks transient; otherwise report provider/network failure and the attempted transport.
